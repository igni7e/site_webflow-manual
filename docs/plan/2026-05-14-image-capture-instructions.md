...
- キャプチャーは原則としてIGNITEが保有するBooostのWebflowサイトを使って撮影する。
- Booost上のサイト名、ユーザー名、メールアドレス、請求情報、フォーム送信者情報、公開前コンテンツは必ずぼかす。
...
2. BooostのWebflowサイトを開く。実データが写る画面では、撮影前にぼかす対象を確認する。
