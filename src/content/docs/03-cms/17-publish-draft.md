...
1.  **CMS管理画面を開く:**
    エディターモードのツールバーから「Collections」を開き、目的のコレクション（例: `お知らせ`）を選択して、記事一覧画面に移動します。
...
5.  **「Publish」ボタンをクリック:**
    ...
    ![公開ボタン](https://uploads-ssl.webflow.com/xxxxxxxx/xxxxxxxx_PublishFromDraft.png)  
    *※画像はイメージです。*
...
**次のステップ:**
これで新規記事の作成から公開までの一連の流れが完了しました。次は、すでに公開されている記事の内容を修正したい場合の「No.34 一度公開した記事を「修正」する方法」です。
