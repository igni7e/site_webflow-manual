...
:::note[キャプチャー指示]
このページには、撮影後に `src/assets/captures/manual/c-19-save-as-draft.png` を入れてください。
撮影対象: C-19「Save as Draft操作」。下書き保存ボタンと状態が見える
...
2.  <strong>「Save as Draft」をクリック:</strong>
    ...
    :::note[キャプチャー差し込み位置]
    ここには「下書き保存ボタン」が分かる実画面キャプチャーを入れてください。ページ上部のキャプチャー指示にある保存ファイル名へ差し替えます。
    :::
