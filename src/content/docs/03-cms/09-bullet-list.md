...
:::note[キャプチャー指示]
このページには、撮影後に `src/assets/captures/manual/c-12-list-menu.png` を入れてください。
撮影対象: C-12「箇条書きメニュー」。Bullet list / Numbered listが分かる
...
> 旧マニュアル番号: [No.25](/03-cms/09-bullet-list/)
...
3.  <strong>フローティングメニューからアイコンを選択:</strong>
    ...
    :::note[キャプチャー差し込み位置]
    ここには「リスト作成メニュー」が分かる実画面キャプチャーを入れてください。ページ上部のキャプチャー指示にある保存ファイル名へ差し替えます。
    :::
...
箇条書きで情報がスッキリしましたね。次は、文章だけでは伝わりにくい情報を補う「[No.26](/03-cms/10-insert-image/) 本文に「写真（画像）」を入れる方法」です。
