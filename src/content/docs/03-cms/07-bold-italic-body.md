...
:::note[キャプチャー指示]
このページには、撮影後に `src/assets/captures/manual/c-10-rich-text-formatting.png` を入れてください。
撮影対象: C-10「太字・斜体メニュー」。Bold、Italic、選択テキストが分かる
...
3.  <strong>フローティングメニューを表示させる:</strong>
    ...
    :::note[キャプチャー差し込み位置]
    ここには「文字装飾メニュー」が分かる実画面キャプチャーを入れてください。ページ上部のキャプチャー指示にある保存ファイル名へ差し替えます。
    :::
