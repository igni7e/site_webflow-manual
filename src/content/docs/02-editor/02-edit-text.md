...
- B-03: `src/assets/captures/manual/b-03-editable-text-hover.png`。編集可能なテキストにカーソルを合わせた状態。編集枠、編集アイコン、対象テキストが見える
- B-04: `src/assets/captures/manual/b-04-text-editing-active.png`。テキストをクリックして入力できる状態。入力カーソル、変更対象の見出しまたは本文が分かる
...
4.  青い枠と鉛筆アイコンを確認:
    ...
    :::note[キャプチャー差し込み位置]
    ここには「テキスト編集の合図」が分かる実画面キャプチャーを入れてください。ページ上部のキャプチャー指示にある保存ファイル名へ差し替えます。
    :::
